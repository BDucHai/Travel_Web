...
const SearchTour = () => {
    ...
    const handleGetTour = async () => {
        try {
            setLoading(true);
            // const res = await getTours({ subtitle: query });
            // setTours(res || []);
            setPage(1);
        } finally {
            setLoading(false);
    ...
    return (
        <div className="min-h-screen bg-[#fcf5ef] px-6 py-10">
            ...
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
                <h1 className="text-4xl md:text-5xl font-bold text-gray-800">{t(`navbar.${query}`)}</h1>
            </motion.div>
                    ...
                    {Array.from({ length: totalPages }).map((_, i) => (
                        <button
                            ...
                            className={`px-3 py-1 rounded-lg border transition ${
                                page === i + 1 ? "bg-black text-white" : "bg-white text-gray-700"
                            }`}>
                            {i + 1}
                        </button>
                    ))}
